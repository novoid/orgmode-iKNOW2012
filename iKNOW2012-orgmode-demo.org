...
R[fn:r-lang]. The data is generated or processed using one computer
language and transferred to the next one which is different from the
previous. With this technique, it is possible to process the data with
the most suitable command language appropriate for this single
processing step.
...
\emph{tagstore}[fn:tagstore-url]\cite{Voit2012a}.
