...
Reproducible Research can be seen as the ultimate aim of Open Science
and Open Data. People do not only get access to the data related to a
research paper but also to the complete process of deriving the paper
from the raw data up to the \myacro{PDF} file of the white paper.
...
In order to use Org-mode to generate a \myacro{PDF} file the user has
to follow these two steps:
...
2. Export to \LaTeX{}/PDF
   - a single command, directly invoked within Emacs or for example
     using \myacro{GNU} make
...
\emph{tagstore}\footnote{http://tagstore.org}\cite{Voit2012a}.
